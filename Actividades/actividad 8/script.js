...
class Calculator {
      ...
      for (const part of parts) {
        ...
        if (assignmentMatch) {
          ...
          try {
            ...
            variables[varName.trim()] = this.evaluateExpression(
              value.trim(),
              variables
            );
          } catch (error) {
            throw new Error(
              `Error al evaluar la variable "${varName}": ${error.message}`
            );
          }
        } else {
  ...
  evaluateExpression(expression, variables) {
    ...
    if (!/^[a-zA-Z0-9+\-*/().\s^]+$/.test(expression)) {
      throw new Error(
        `Expresión contiene caracteres no válidos: ${expression}`
      );
    }
  ...
  calculate() {
    try {
      ...
      if (!variablesText && !equationsText) {
        this.showResult(
          "Por favor ingresa las variables y ecuaciones",
          "error"
        );
        return;
      }
      ...
      for (const equation of equations) {
        ...
        if (equationMatch) {
          ...
          try {
            ...
            throw new Error(
              `Error en ecuación "${trimmedEquation}": ${error.message}`
            );
          }
        } else {
          ...
          try {
            ...
            throw new Error(
              `Error al evaluar "${trimmedEquation}": ${error.message}`
            );
          }
        }
      }
      ...
      if (results.length === 0) {
        this.showResult(
          "No se encontraron ecuaciones válidas para calcular",
          "warning"
        );
      } else {
  ...
  showResult(message, type = "success") {
    ...
    this.resultDiv.classList.remove(
      "result-success",
      "result-error",
      "result-warning"
    );
